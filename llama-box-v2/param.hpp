...
static bool llama_box_params_parse(int argc, char **argv, llama_box_params &params_) {
        ...
        for (int i = 1; i < argc;) {
            ...
            if (!strcmp(flag, "-ot") || !strcmp(flag, "--override-tensor")) {
                ...
                for (const auto &override : string_split<std::string>(std::string(arg), ',')) {
                    ...
                    params_.hs_params.llm_params.tensor_buft_overrides.push_back({strdup(tensor_name.c_str()), buffer_types.at(buffer_type)});
                }
            }
            ...
            if (!strcmp(flag, "-mg") || !strcmp(flag, "--main-gpu")) {
                ...
                if (params_.hs_params.llm_params.main_gpu < 0 || params_.hs_params.llm_params.main_gpu >= int32_t(llama_max_devices())) {
                    invalid("--main-gpu");
                }
                continue;
            }
            ...
            if (!strcmp(flag, "--prio")) {
                ...
                char *arg                                       = argv[i++];
                params_.hs_params.llm_params.cpuparams.priority = (enum ggml_sched_priority)std::stoi(std::string(arg));
                continue;
            }
            ...
            if (!strcmp(flag, "--prio-batch")) {
                ...
                char *arg                                             = argv[i++];
                params_.hs_params.llm_params.cpuparams_batch.priority = (enum ggml_sched_priority)std::stoul(std::string(arg));
                continue;
            }
            ...
            if (!strcmp(flag, "--image-strength")) {
                ...
                if (params_.hs_params.sd_params.sampling.strength < 0.0f || params_.hs_params.sd_params.sampling.strength > 1.0f) {
            ...
            if (!strcmp(flag, "--image-control-strength")) {
                ...
                if (params_.hs_params.sd_params.sampling.control_strength < 0.0f || params_.hs_params.sd_params.sampling.control_strength > 1.0f) {
    ...
    // Postprocess params
    if (params_.hs_params.llm_params.chat_template.size() > 20 && !common_chat_verify_template(params_.hs_params.llm_params.chat_template, params_.hs_params.llm_params.use_jinja)) {
        invalid("--chat-template");
    }
    ...
    postprocess_cpu_params(params_.hs_params.llm_params.speculative.cpuparams_batch, &params_.hs_params.llm_params.cpuparams_batch);
