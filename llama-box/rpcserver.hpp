...
bool rpcserver::copy_tensor(const std::vector<uint8_t> &input, std::vector<uint8_t> &output) {
    ...
    if (src == nullptr) {
        SRV_ERR("failed to deserialize src tensor: id = %lu, name = %s, type = %s\n", rpc_src->id, rpc_src->name, ggml_type_name((ggml_type)rpc_src->type));
        return false;
    }
    ggml_tensor *dst = deserialize_tensor(ctx, rpc_dst);
    if (dst == nullptr) {
        SRV_ERR("failed to deserialize dst tensor: id = %lu, name = %s, type = %s\n", rpc_dst->id, rpc_dst->name, ggml_type_name((ggml_type)rpc_dst->type));
        delete src;
        return false;
    }
...
bool rpcserver::support_op(const std::vector<uint8_t> &input, std::vector<uint8_t> &output) {
    ...
    if (tensor == nullptr) {
        SRV_ERR("failed to deserialize tensor: id = %lu, name = %s, type = %s\n", tensors[n_tensors - 1].id, tensors[n_tensors - 1].name,
                ggml_type_name((ggml_type)tensors[n_tensors - 1].type));
        return false;
    }
...
ggml_tensor *rpcserver::create_node(uint64_t id, struct ggml_context *ctx, const std::unordered_map<uint64_t, const rpc_tensor *> &in_tensor_ptrs,
