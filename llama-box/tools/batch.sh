...
function request() {
  ...
  # result
  tps=$(echo "scale=2; $tts * 1000 / $tt" | bc)
  avg_pps=$(echo "scale=2; $pts * 1000 / $tt / $oks" | bc)
  avg_dps=$(echo "scale=2; $dts * 1000 / $tt / $oks" | bc)
  if [[ "${ppss}" != "0" ]]; then
    avg_pps=$(echo "scale=2; $ppss / $oks" | bc)
  fi
  if [[ "${dpss}" != "0" ]]; then
    avg_dps=$(echo "scale=2; $dpss / $oks" | bc)
  fi
