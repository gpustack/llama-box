...
static void llama_box_params_print_usage(int, char **argv, const llama_box_params &bparams) {
    ...
    opts.push_back({ "server/completion",  "       --cache-reuse N",        "min chunk size to attempt reusing from the cache via KV shifting (default: %d)", bparams.n_cache_reuse });
...
static bool llama_box_params_parse(int argc, char **argv, llama_box_params &bparams) {
    try {
        for (int i = 1; i < argc;) {
            ...
            if (!strcmp(flag, "--cache-reuse")) {
                ...
                char *arg = argv[i++];
                bparams.n_cache_reuse = std::stoi(std::string(arg));
                continue;
            }
    ...
    get_env("LLAMA_ARG_CACHE_REUSE", bparams.n_cache_reuse);
